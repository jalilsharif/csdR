...
all genes within each condition using a correlation measure
(Spearman by default; Pearson or biweight midcorrelation via `method`).
Optional confounder residualization and Schäfer–Strimmer shrinkage can be
applied before / after estimating the mean correlations.
For each pair of genes, we apply bootstrapping across the samples
and compute the mean correlation
...
Approximate BH FDR columns (`sFDR`, `dFDR`, …) are attached automatically.
For a more trustworthy empirical FDR, use `csd_permutation_fdr()`.
To label gene pairs (including which condition drives an S-edge), use
`annotate_csd()` or `select_top_csd()`.
